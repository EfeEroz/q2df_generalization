program q2df_generalization
    ...
    num1 = -1.0_WP / 92.14_WP * 9.0_WP * 31.999_WP
    num2 = 1.0_WP * 0.23292_WP
    num3 = -1.0_WP / 100.21_WP * 11.0_WP * 31.999_WP

    ! call indiv_test_get_optimal_bcs(0.0_WP, 0.0_WP, 0.0_WP, 0.0_WP, 0.0_WP, num1, num2, num3)
    ! call indiv_test_get_optimal_bcs(0.0_WP, 0.1_WP, 0.3_WP, 0.1_WP, 0.2_WP, num1, num2, num3)
...
subroutine get_optimal_bcs(Z1_CFD, Z2_CFD, chi11_CFD, chi12_CFD, chi22_CFD, leftover_ox1_CFD, &
    ...
    integer :: m, n
    real(WP) :: x_opt_candidate, chi_ratio
    integer :: k
    
    real(WP) :: q2df2_chi_ratio, q2df2_xi, q2df2_eta, q2df2_chi_ratio_check, generalized_chi_ratio, abs_diff, per_diff
    ...
    if (leftover_ox1_CFD * leftover_ox2_CFD > 0.0_WP .and. leftover_ox1_CFD * leftover_ox3_CFD > 0.0_WP) then
        print *, "No stoichiometric mixtures. Nonpremixed combustion cannot occur."
        print *, "Leftover oxygen amounts", leftover_ox1_CFD, leftover_ox2_CFD, leftover_ox3_CFD
    end if
    ...
    chi13_CFD = -chi11_CFD_new - chi12_CFD_new
    chi23_CFD = -chi22_CFD_new - chi12_CFD_new
    chi33_CFD = chi11_CFD_new + 2.0_WP*chi12_CFD_new + chi22_CFD_new
    chi(1,1) = chi11_CFD_new; chi(1,2) = chi12_CFD_new; chi(1, 3) = chi13_CFD
    chi(2,1) = chi12_CFD_new; chi(2,2) = chi22_CFD_new; chi(2, 3) = chi23_CFD
    ...
    abs_diff = abs(q2df2_chi_ratio - q2df2_chi_ratio_check)
    per_diff = abs_diff / q2df2_chi_ratio * 100.0_WP
    if (per_diff > 1.0_WP) then   ! Made less restrictive to account for robustness
...
subroutine get_x_extrema(Z1, Z2, chi11, chi12, chi22, x_extrema)
    ...
    do i = 1, GRID
        ...
        if (chi_ratio > -0.5_WP) then
            ...
            x_val_arr(counter) = start_pos + step_size*(i-1)
        end if
    end do
    ...
    do i = 2, counter-1
        if (chi_ratio_arr(i) < chi_ratio_arr(i-1) .and. chi_ratio_arr(i) < chi_ratio_arr(i+1)) then
            call append(x_extrema, x_val_arr(i))
        end if
    end do
...
subroutine indiv_test_get_optimal_bcs(Z1_CFD, Z2_CFD, chi11_CFD, chi12_CFD, chi22_CFD, leftover_ox1_CFD, &
    leftover_ox2_CFD, leftover_ox3_CFD)
    implicit none
    ...
    print *, "    OMIX =", OMIX
    print *, "    FMIX =", FMIX
    print *, "    chi_xi =", chi_xi, "chi_eta =", chi_eta, "x_prime =", x_prime, "xi_opt =", Z_opt, "eta_opt =", &
        eta_opt, "Z_stoic =", Z_stoic
end subroutine indiv_test_get_optimal_bcs
